...
max_epoch = 0
start_epoch = 100
weight_files = glob.glob('weights/corr_weights1_1_output_*')
...
print('start_epoch, max_epoch: {}, {}'.format(start_epoch, max_epoch))
weight_names = ['weights1_1', 'weights1_2', 'weights3_1', 'weights3_2'] #'weights1_1', 'weights1_2', 'weights3_1', 'weights3_2'

for weight_name in weight_names:
    # [# cut-off epoch, # weights]
    epoch_values = []
    for i in range(start_epoch, num_docs):
        values = readAllWeights('./weights/corr_{}_output_{}'.format(weight_name, i))
        nums = len(values)
        # print(nums)
        if nums == 0:
    ...
    for i in range(1, len(epoch_values)):
        ...
        # print('len gradient_values: {}'.format(len(gradient_values)))
        gradient_epoch_values.append(gradient_values)

    # print('len(epoch_values[i]), len(gradient_epoch_values): {}'.format(len(epoch_values[i])), len(gradient_epoch_values))
    # print('len(gradient_epoch_values): {}, len(gradient_epoch_values[0]): {}'.format(len(gradient_epoch_values), len(gradient_epoch_values[0])))
    corr_array = np.array(gradient_epoch_values)
    # corr_array = corr_array.reshape((len(gradient_epoch_values), len(epoch_values[i])))
    # test = np.array(test)
    # print('sum test: {}'.format(np.sum(test)))
    corr_array = np.transpose(corr_array)
    corr_array_shape = np.shape(corr_array)
    print('corr_array_shape: {}'.format(corr_array_shape))
