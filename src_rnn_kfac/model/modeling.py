...
def lstm(sentence, params, var_scope='cnn'):
    ...
    with tf.variable_scope(var_scope, reuse=tf.AUTO_REUSE):
        ...
        embeddings = tf.get_variable(name="embeddings", dtype=tf.float32,
                ...
                initializer=tf.truncated_normal_initializer(stddev=1e-1), trainable=False)
        # embeddings = tf.get_variable(name="embeddings", dtype=tf.float32,
        #         shape=[params.vocab_size, params.embedding_size])
        sentence = tf.nn.embedding_lookup(embeddings, sentence)
        # Apply LSTM over the embeddings
        lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(params.lstm_num_units, trainable=False)
        ...
        logits = tf.layers.dense(last_output, params.num_classes, trainable=trainable)#state[1]
...
def retrain_lstm(inputs, params=None, var_scope='cnn'):
    ...
    with tf.variable_scope(var_scope, reuse=tf.AUTO_REUSE):
        ...
        # only the final output
        last_output = outputs[:, -1]
        Ylogits = tf.layers.dense(last_output, params.num_classes, trainable=trainable)
    ...
    with tf.variable_scope("mask", reuse=tf.AUTO_REUSE):
        ...
        # only the final output
        last_mask_output = mask_outputs[:, -1]
        logits = tf.layers.dense(last_mask_output, params.num_classes, trainable=False)  
        ...
        if is_training:
            if params.use_kfac:
                ...
                with tf.name_scope('kfac_optimizer'):
                    ...
                    # layer_collection.auto_register_layers(var_list=cnn_vars)
                    # c_cnn_vars=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='model/c_cnn')
                    # layer_collection.auto_register_layers(var_list=c_cnn_vars, batch_size=params.batch_size)                    
                    # Construct training ops
                    global_step = tf.train.get_or_create_global_step()
                    optimizer = kfac.PeriodicInvCovUpdateKfacOpt(learning_rate=params.learning_rate, damping=0.001, \
                        batch_size=params.batch_size, layer_collection=layer_collection)
