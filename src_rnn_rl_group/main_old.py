...
if __name__ == '__main__':
    ...
    if params.num_learners <= 1:# not args.retrain or args.combine:
        if args.combine:
            ...
            params.dict['train_size'] = cal_train_size(params.train_size, args.train_range) * 2
        ...
        npy_vars_weights1_1, npy_vars_weights1_2, npy_vars_weights3_1, npy_vars_weights3_2 = [], [], [], []
        if args.finetune:
            npy_vars_weights1_1 = np.load('cluster_array_rnn_basic_lstm_cell_kernel.npy')
            # npy_vars = np.reshape(npy_vars, shape)
            npy_vars_weights1_2 = np.load('cluster_array_rnn_basic_lstm_cell_bias.npy')
            npy_vars_weights3_1 = np.load('cluster_array_dense_kernel.npy')
            npy_vars_weights3_2 = np.load('cluster_array_dense_bias.npy')
        npy_vars = [npy_vars_weights1_1, npy_vars_weights1_2, npy_vars_weights3_1, npy_vars_weights3_2]    
        train_model_spec = model_fn('train', train_inputs, params, npy_vars=npy_vars)
        eval_model_spec = model_fn('vali', eval_inputs, params, npy_vars=npy_vars, reuse=True)
        logging.info("- done.")
        logging.info("Starting training for at most {} epoch(s) for the initial learner".format(params.num_epochs))
        start_time = time.time()
        # global_epoch = train_and_evaluate_sample(train_model_spec, eval_model_spec, \
        #     eval_train_model_spec, args.model_dir, params, \
