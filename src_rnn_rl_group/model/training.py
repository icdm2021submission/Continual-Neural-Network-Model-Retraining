...
def train_sess(sess, model_spec, num_steps, writer, params, \
    ...
    for i in t:
        ...
        # loss_val = 0
        # x = 0
        # for w in gradients_vars:
        #     print('index x: {}, Gradient shape: {}'.format(x, np.shape(w)))
        #     x +=1        
        
        for g in gradients_vars:
            flat = np.square(g).flatten()
            for f in flat:
                loss_val += np.sum(f)
        epoch_loss.append(loss_val)
        # #################################################################################
        if 'retrain' in params.loss_fn and params.finetune:
            global_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='model/rewards')
            action_var=[v for v in global_vars if 'action' in v.name][0]
            action_var_value = sess.run([action_var])
            logging.info('****action_var at train_sess:\n {}'.format(action_var_value))
...
def save_var(sess, name, epoch):
    cnn_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="{}".format(name))[0]
    cnn_vars = sess.run(cnn_vars)
    print(cnn_vars.shape)
    cnn_vars = cnn_vars.flatten().tolist()
    name = name.replace('model/cnn/', '').replace('/', '_').replace(':0', '')
    save_vars_to_file(cnn_vars, "./weights/corr_weights{}_output_{}".format(name, epoch))
...
def train_and_evaluate(train_model_spec, eval_model_spec,
    ...
    with tf.Session() as sess:
        ...
        model_summary()
        # for each learner
        num_train_steps = 3#(params.train_size + params.batch_size - 1) // params.batch_size
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            names = [v.name for v in tf.trainable_variables() if 'model/cnn' in v.name and 'embeddings' not in v.name]
            logging.info(names)
            for name in names:
                save_var(sess, name, epoch)                             
            ...
            if isSavingWeights(eval_metrics, best_eval_metrics):
                ...
                if params.loss_fn == 'cnn' and not params.use_kfac:
                    cnn_vars=[v for v in tf.trainable_variables() if 'model/cnn' in v.name and 'embeddings' not in v.name]
                    c_cnn_vars=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='model/c_cnn')
                    c_cnn_vars = [v for v in c_cnn_vars if 'model/cnn' in v.name and 'embeddings' not in v.name]
                    # c_cnn_vars=[v for v in tf.trainable_variables() if 'model/c_cnn' in v.name and 'embeddings' not in v.name]
                    update_weights = [tf.assign(c, old) for (c, old) in \
                    zip(c_cnn_vars, cnn_vars)]
                    sess.run(update_weights)
                    best_save_path = os.path.join(model_dir, 'best_weights', 'after-epoch')
