...
def collect_lenet(inputs, is_training, params=None, var_scope='cnn', trainable=True):
    ...
    with tf.name_scope('conv1_1'):
        with tf.variable_scope(var_scope, reuse=tf.AUTO_REUSE):
            filter1_1 = tf.get_variable('weights1_1', shape=[depth, 16], \
        ...
        with tf.variable_scope('mask', reuse=tf.AUTO_REUSE):
            mask_filter1_1 = tf.get_variable('mweights1_1', shape=[depth, 16], \
    ...
    with tf.name_scope('conv1_2'):
        ...
        with tf.variable_scope('mask', reuse=tf.AUTO_REUSE):
            ...
            mask_biases1_2 = tf.get_variable('mbiases1_2', shape=[4], \
                initializer=tf.zeros_initializer(), trainable=trainable)   
        # filter1_2 = tf.multiply(cluster_filter1_2, filter1_2)           
...
def retrain_lenet(inputs, is_training, params=None, var_scope='cnn', npy_vars=None):
    ...
    with tf.name_scope('conv1_1'):
        ...
        cluster_biases = tf.ones(shape=[16])
    ...
    with tf.name_scope('conv1_2'):
        ...
        cluster_biases = tf.ones(shape=[4])
        cluster_biases = tf.cast(cluster_biases, dtype=tf.float32)
    ...
    with tf.name_scope('fc2') as scope:
        ...
        cluster_fc2w = tf.ones(shape=[4, params.num_classes])
...
def get_mask(mask_biases, action):
    ...
    mask_biases = tf.cond(tf.equal(tf.reduce_sum(mask_biases), 0.0), lambda:mask_biases, lambda:tf.cast(tf.ones_like(mask_biases), dtype=tf.float32))
    return mask_biases
...
def lenet(X, params=None, var_scope='cnn'):
    ...
    with tf.variable_scope(var_scope, reuse=tf.AUTO_REUSE):
        ...
        with tf.name_scope('conv1_1'):
            filter1_1 = tf.get_variable('weights1_1', shape=[depth, 16], \
                initializer=tf.truncated_normal_initializer(stddev=1e-1))
            mask_filter1_1 = tf.random_uniform(shape=[depth, 16])
            # filter1_1 = tf.Print(filter1_1, [filter1_1], '***************************filter1_1')
            mask_filter1_1 = get_random_mask(mask_filter1_1)
        ...
        with tf.name_scope('conv1_2'):
            ...
            mask_filter1_2 = get_random_mask(mask_filter1_2)
        ...
        with tf.name_scope('fc2') as scope:
            ...
            mask_fc2w = get_random_mask(mask_fc2w)
    ...
    with tf.variable_scope(var_scope, reuse=tf.AUTO_REUSE):
        ...
        with tf.name_scope('fc2') as scope:
            ...
            fc2b = tf.get_variable('biases3_2', shape=[params.num_classes], \
                initializer=tf.constant_initializer(0.0))
            Ylogits = tf.nn.bias_add(tf.matmul(fc1_drop, fc2w), fc2b)
            # mask_fc2w = tf.convert_to_tensor(npy_vars[2], dtype=tf.float32)
            mask_fc2w = tf.ones(shape=[4, params.num_classes])
            # mask_fc2w = get_mask(mask_fc2w, action)
            cluster_biases = tf.ones(shape=[params.num_classes])
            # cluster_biases = get_mask(cluster_biases)
            cluster_biases = tf.cast(cluster_biases, dtype=tf.float32)
...
def build_model(mode, inputs, params, npy_vars):
    ...
    # if 'reward' not in inputs:
    #     inputs['reward'] = 0
    # else:
    #     inputs['reward'] += inputs['reward']
...
def model_fn(mode, inputs, params, npy_vars=None, reuse=False):
    ...
    with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
        ...
        # calcualted_loss = tf.Print(calcualted_loss, [calcualted_loss], message='calcualted_loss\n')
        # group weights for initial training
        if params.group:
    ...
    if is_training:
        ...
        model_spec['gradients'] = gradients
    return model_spec
