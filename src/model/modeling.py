    ...
    with tf.variable_scope(var_scope, reuse=tf.AUTO_REUSE):
        ...
        with tf.name_scope('pool1'):
            pool1_1 = tf.nn.max_pool(conv1_1,
                                     ...
                                     padding='SAME',
                                     name='pool1_1')
            pool1_1_drop = pool1_1
            if params.training_keep_prob:
                pool1_1_drop = tf.nn.dropout(pool1_1_drop, params.training_keep_prob)
        # CONVOLUTION 1 - 2
        with tf.name_scope('conv1_2'):
            filter1_2 = tf.get_variable('weights1_2', shape=[5, 5, 32, 64], \
                initializer=tf.truncated_normal_initializer(stddev=1e-1))
        ...
        with tf.name_scope('pool2'):
            pool2_1 = tf.nn.max_pool(conv1_2,
                                     ...
                                     padding='SAME',
                                     name='pool2_1')
            pool2_1_drop = pool2_1
            if params.training_keep_prob:
                pool2_1_drop = tf.nn.dropout(pool2_1_drop, params.training_keep_prob)
        #FULLY CONNECTED 1
        with tf.name_scope('fc1') as scope:
            pool2_flat = tf.layers.Flatten()(pool2_1_drop)
            dim = pool2_flat.get_shape()[1].value
            ...
            fc1_drop = fc1
            if params.training_keep_prob:
                fc1_drop = tf.nn.dropout(fc1_drop, params.training_keep_prob)
        #FULLY CONNECTED 2
        with tf.name_scope('fc2') as scope:
            fc2w = tf.get_variable('weights3_2', shape=[1024, params.num_classes], \
                initializer=tf.truncated_normal_initializer(stddev=1e-1))
