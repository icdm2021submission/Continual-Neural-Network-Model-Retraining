...
def convert_to_tf_record(data_directory:str):
    ...
    #z-score
    mean = np.mean(mnist.train.images,axis=(0,1,2,3))
    std = np.std(mnist.train.images,axis=(0,1,2,3))
    # x_train = (x_train-mean)/(std+1e-7)
    # x_test = (x_test-mean)/(std+1e-7)
    num_validation_examples, rows, cols, depth = convert_to(mnist.validation, 'validation', data_directory, mean, std, num_shards=10)
    num_train_examples, rows, cols, depth = convert_to(mnist.train, 'train', data_directory, mean, std, num_shards=10)
    num_test_examples, rows, cols, depth = convert_to(mnist.test, 'test', data_directory, mean, std, num_shards=10)
