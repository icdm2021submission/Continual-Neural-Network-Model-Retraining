...
def plot(distortions, num_clusters):
    plt.clf()
    plt.plot(range(1, num_clusters), distortions, marker='o', linewidth=4)
    # new_xticks = list(range(0, num_clusters, 2))
    # new_xticks = new_xticks[1:]
    # plt.xticks(new_xticks, [str(v) for v in new_xticks])
    plt.xticks(np.arange(0, num_clusters))
    plt.xlabel('Number of clusters')
    plt.ylabel('SSE')
    plt.savefig('SEA_scree_cluster_weights1_1.pdf')
