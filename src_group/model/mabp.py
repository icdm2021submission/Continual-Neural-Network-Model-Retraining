...
def rl_weights(params, numbers_of_selections, sums_of_reward, max_upper_bound, n, arm_weights):
	indexes = list(range(0, len(sums_of_reward)))
	np_sums_of_reward = np.array(sums_of_reward)
	if params.rl == 'Greedy':
	    chosen_arm = np.argmax(np_sums_of_reward)
	    # reward = sums_of_reward[chosen_arm]
	...
	if params.rl == 'Reservoir':
	    ...
	    numbers_of_selections[chosen_arm] += 1
	    # logging.info('numbers_of_selections in mabp.py: {}'.format(numbers_of_selections))
	    return chosen_arm, numbers_of_selections, None, None	    
	if params.rl == 'EI':
	    ...
	    return chosen_arm, numbers_of_selections, None, None
	if params.rl == 'EI2':
		largest_index, largest2_index = top2(sums_of_reward)
		# logging.info('largest_index: {}, largest2_index : {}'.format(largest_index, largest2_index ))
	...
	if params.rl == 'UCB':
		...
		# reward = sums_of_reward[chosen_arm]
		numbers_of_selections[chosen_arm] += 1
		# sums_of_reward[chosen_arm] += sums_of_reward[chosen_arm]
		return chosen_arm, numbers_of_selections, None, max_upper_bound
	if params.rl == 'TS':
		sum_ = sum(sums_of_reward)
		sums_of_reward = [float(v/sum_) for v in sums_of_reward]
		bandit_priors = [
		...
		chosen_arm = 1
		return chosen_arm, numbers_of_selections, sums_of_reward, None
	if params.rl == 'EXP3':
		n_arms = len(indexes)
		...
		return chosen_arm, numbers_of_selections, arm_weights, None
	if params.rl == 'EXP4':
		n_arms = len(indexes)
		probs = [0.0 for i in indexes]
		total_weight = sum(arm_weights)
