...
def readOneFile(file_path):
  with open(file_path, 'r') as f:
    lines = f.readlines()
    values = [float(v.strip()) for v in lines]
  return values

weight_names = ['weights3_1']#'weights1_1', 'weights1_2', 'weights3_1', 'weights3_2'
# gap = 10
num_docs = 170
for weight_name in weight_names:
  epoch_values = []
  for i in range(131, num_docs+1):
    values = readOneFile('corr_{}_output_{}'.format(weight_name, i))
    epoch_values.append(values)
  corr_array = np.array(epoch_values)
corr_array = np.transpose(corr_array)
corr_array_shape = corr_array.shape
print(corr_array.shape)
np.save(weight_name, corr_array)

# corr_array_shape = (800, 40)#CNN 1
# (51200, 40) #CNN 2
X = np.load('{}.npy'.format(weight_name)).reshape(corr_array_shape)
