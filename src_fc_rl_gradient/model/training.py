...
def train_sess(sess, model_spec, num_steps, writer, params):
    ...
    for i in t:
        ...
        # loss_val = np.sum(np.square(gradients))
        # loss_val = np.sum(gradients)
        # logging.info(loss_val)
        # logging.info(type(loss_val))
...
def train_and_evaluate_sample(train_model_spec, eval_model_spec, eval_train_model_spec,
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            global_epoch += 1
        begin_at_epoch = global_epoch
        second_round = begin_at_epoch + params.num_epochs2
        for epoch in range(begin_at_epoch, second_round):
            # Run one epoch
            logging.info("Epoch {}/{}".format(epoch + 1, \
                second_round))
