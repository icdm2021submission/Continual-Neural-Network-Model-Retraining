...
def input_fn(mode, dataset, params):
    ...
    features = tf.reshape(features, [-1, \
        depth])
    # logging.info(tf.shape(features))
...
if __name__ == "__main__":
    ...
    dataset_files = os.path.join(args.data_dir, 'sample-temp.tfrecords')#train-*
    ...
    with tf.Session() as sess:
        ...
        for i in range(20):
            try:
                print(sess.run([tf.shape(features), tf.shape(labels), labels]))
